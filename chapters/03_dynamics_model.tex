...
\begin{enumerate}
  \item To define the dynamic relationships and notation used in subsequent analysis and simulation.
  \item To quantify the order of magnitude of aerodynamic forces that the INDI controller must reject without an explicit model.
\end{enumerate}

The controller described in Chapter~\ref{chapter:control-architecture} relies solely on incremental acceleration feedback; none of the aerodynamic coefficients or equations derived here are required in real time.
...
The cubic relation is used solely as a qualitative reference. Electrical current and torque were not measured; thus, numerical efficiency claims in later chapters are limited to thrust-based proxies.
