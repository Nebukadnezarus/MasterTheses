...
Each rotor $i$ produces a thrust $f_i$ approximately along $+\mathbf{e}_3$ of $\mathcal{B}$ (tilt effects handled in allocation). The net thrust in $\mathcal{W}$:
\begin{equation}
\mathbf{F}_T \;=\; \Big(\sum_{i=1}^4 f_i\Big)\, R\,\mathbf{e}_3.
\end{equation}
...
Let $\mathbf{r}_i$ be the arm vector from CoG to rotor $i$, and $c_i\in\{+1,-1\}$ the prop rotation sign (reaction torque direction). With small motor tilt handled by per-rotor thrust directions $\hat{\mathbf{t}}_i$,
\begin{align}
