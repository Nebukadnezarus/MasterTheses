...
Motors are tilted outward by $5^{\circ}$ to enhance yaw torque. The corresponding geometry is embedded in the allocation matrix used by the controller; no additional coupling compensation is required. Static thrust characterization (Chapter~\ref{chapter:experiments-evaluation}, Section~\ref{sec:thrust_map_results}) confirms a quadratic thrust--RPM relation with $R^2 = 0.997$, validating consistency between analytical assumptions and measured performance.
...
A second airframe variant was later developed to improve aerodynamic efficiency and stability in forward flight.
...
This shows a deliberate engineering evolution---not an inconsistency.
