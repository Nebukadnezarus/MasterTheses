...
After removing a constant $-0.996$\,N sensor bias, a quadratic model was fitted:
\begin{equation}
...
\begin{figure}[htbp]
  ...
  \begin{axis}[tumplot, xlabel={Command [\%]}, ylabel={RPM [rev/min]}, width=0.8\textwidth]
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{RPM vs Command with linear fit: $\text{RPM} = 19.26 \cdot \text{cmd} + 3.65$.}
...
\subsection{Step-Response Evaluation}\label{sec:step-response}

Step responses in the $x$ and $z$ axes were used to compare response dynamics to a reference geometric controller.
Due to limited data, only qualitative assessment is possible: INDI exhibited visibly faster disturbance rejection and smoother attitude recovery after step commands, consistent with its incremental disturbance-cancellation principle.
...
Closed-loop tracking was evaluated on circular trajectories at cruise speeds between 3\,m\,s$^{-1}$ and 8\,m\,s$^{-1}$.
Vicon data show that the controller maintained stable flight and accurate path following across the entire speed range.
The root-mean-square (RMS) position error increased from $\approx 0.03$\,m at 3\,m\,s$^{-1}$ to $\approx 0.16$\,m at 8\,m\,s$^{-1}$, while maximum deviation remained below 0.45\,m (Figure~\ref{fig:tracking_error_summary}).
...
\begin{figure}[htbp]
  ...
  \begin{axis}[tumplot, 
    ...
    legend pos=north west]
      % Placeholder for tracking error data
      \addplot[thick, TUMBlue, mark=*, mark size=2pt] coordinates {
        (3, 0.03) (4, 0.05) (5, 0.08) (6, 0.11) (7, 0.14) (8, 0.16)
      };
      \addlegendentry{RMS error}
      \addplot[thick, TUMAccentOrange, mark=square*, mark size=2pt] coordinates {
        (3, 0.08) (4, 0.14) (5, 0.22) (6, 0.31) (7, 0.38) (8, 0.45)
      };
      \addlegendentry{Max error}
    \end{axis}
  \end{tikzpicture}
  \caption{Tracking performance vs. cruise speed. RMS error remains sub-decimeter up to 8\,m/s.}
  \label{fig:tracking_error_summary}
\end{figure}

\subsection{Agility and Disturbance Rejection}\label{sec:agility}

The vehicle remained controllable throughout the tested acceleration range, sustaining coordinated turns up to $\approx 0.8$\,g lateral acceleration without attitude saturation or oscillation.
...
Using the analytical model from Chapter~\ref{chapter:platform-design}, the expected lift and drag at 12\,m\,s$^{-1}$ are $L \approx 27$\,N and $D \approx 0.7$\,N.
The measured attitude during cruise showed no sustained pitch bias beyond 3--4°, implying that the wings indeed support the majority of the weight.

Although electrical power was not measured, the reduction in required collective thrust observed in the log-derived RPM data suggests a significant unloading of the rotors in forward flight.
Future work should include current sensing to quantify the exact power reduction.
...
\item Tracking accuracy remains within sub-decimeter RMS error up to 8\,m\,s$^{-1}$.
...
% Experimental data - different wing area assumptions
\addplot[only marks, mark=*, mark size=4pt, TUMAccentGreen, mark options={solid}] 
  table[x=alpha_deg, y=CL_0.3, col sep=comma] {data/aerodynamic_analysis/experimental_aerodynamic_coefficients.csv};
\addlegendentry{Exp. ($S_{\text{eff}} = 0.3 S$)}
\addplot[only marks, mark=square*, mark size=4pt, TUMAccentGreen, mark options={solid}] 
  table[x=alpha_deg, y=CL_0.5, col sep=comma] {data/aerodynamic_analysis/experimental_aerodynamic_coefficients.csv};
\addlegendentry{Exp. ($S_{\text{eff}} = 0.5 S$)}
\addplot[only marks, mark=triangle*, mark size=4pt, TUMAccentGreen, mark options={solid}] 
  table[x=alpha_deg, y=CL_1.0, col sep=comma] {data/aerodynamic_analysis/experimental_aerodynamic_coefficients.csv};
\addlegendentry{Exp. ($S_{\text{eff}} = S$)}
\end{axis}
\end{tikzpicture}
\caption{Lift coefficient comparison between NACA~0015 and AG25 airfoils at two Reynolds numbers ($Re=1\times10^5$ and $Re=5\times10^5$), computed using XFLR5 with NCrit=5. Experimental data points from flight tests (green markers) are shown with three different effective wing area assumptions used in the $C_L$ calculation to account for observed lift deficiency. All experiments were conducted with the same physical wing; the variations represent different hypotheses about the aerodynamically effective area. The $S_{\text{eff}} = 0.5 S$ assumption shows the best agreement with XFLR5 predictions at operational angles of attack, suggesting that approximately half of the geometric wing area contributes effectively to lift generation. At $Re=1\times10^5$, the AG25 achieves maximum \(C_L \approx 1.20\) at \SI{10}{\degree}, while the NACA~0015 reaches \(C_L \approx 0.97\) at \SI{12}{\degree}.}
...
The table demonstrates progressive rotor unloading with increasing flight speed: at \SI{9.1}{\meter\per\second}, the platform achieves 16\% power savings, increasing to 43\% at \SI{13.6}{\meter\per\second}. Notably, the pitch angle decreases from \SI{37.2}{\degree} to \SI{16.3}{\degree} as speed increases, indicating that the wings generate sufficient lift at higher speeds to maintain altitude with a more level flight attitude. This trend validates the $V^2$ scaling of aerodynamic lift: as forward speed increases, wing-generated lift grows quadratically, allowing the rotors to spin progressively slower while maintaining altitude.

\paragraph{Energy savings calculation.}
Representative calculations at \SI{9.1}{\meter\per\second} and \SI{13.6}{\meter\per\second} illustrate the efficiency trends:

\textbf{Wing~1 (NACA~0015) at \SI{9.1}{\meter\per\second}:}
\begin{align}
\text{RPM ratio} &= \frac{\text{RPM}_{\text{cruise}}}{\text{RPM}_{\text{hover}}} = \frac{1130}{1200} = 0.942 \\
\text{Power ratio} &\approx \left(\frac{1130}{1200}\right)^3 = 0.836 \\
\text{Power savings} &\approx 1 - 0.836 = 16.4\%
\end{align}

\textbf{Wing~1 (NACA~0015) at \SI{13.6}{\meter\per\second}:}
\begin{align}
\text{RPM ratio} &= \frac{\text{RPM}_{\text{cruise}}}{\text{RPM}_{\text{hover}}} = \frac{997}{1200} = 0.831 \\
\text{Power ratio} &\approx \left(\frac{997}{1200}\right)^3 = 0.574 \\
\text{Power savings} &\approx 1 - 0.574 = 42.6\%
\end{align}

\textbf{Wing~2 (AG25) at \SI{9}{\meter\per\second}:}
\begin{align}
...
\paragraph{Power-estimation limitations.}
As derived in Section~\ref{sec:rotor-power}, the $P \propto \Omega^3$ relation holds under hover conditions where induced power dominates. In the present forward-flight case this assumption no longer applies; the estimate therefore serves only as an upper bound.
...
\begin{itemize}
  \item \textbf{NACA~0015 baseline}: Achieves approximately 16\% power savings at \SI{9.1}{\meter\per\second} cruise compared to a conventional quadcopter maintaining the same altitude and speed. At higher speed (\SI{13.6}{\meter\per\second}), the savings increase dramatically to 43\%, demonstrating that efficiency gains scale with flight speed as wing-generated lift increases with $V^2$.
  
  \item \textbf{AG25 optimized}: Delivers approximately 50\% power reduction at \SI{9}{\meter\per\second} cruise—essentially halving the required rotor power compared to a wingless quadcopter. This dramatic improvement validates the airfoil optimization effort and demonstrates the platform's potential for extended endurance missions.
