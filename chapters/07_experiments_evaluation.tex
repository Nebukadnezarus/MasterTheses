...
Flight tests were conducted at speeds ranging from \SI{9.14}{\meter\per\second} to \SI{13.6}{\meter\per\second} with pitch angles from \SI{37.2}{\degree} to \SI{16.3}{\degree}. The extracted coefficients are integrated into Figures~\ref{fig:cl_comparison}--\ref{fig:ld_comparison} as green markers, showing:

\begin{itemize}
  \item \textbf{Lift coefficients}: Experimental $C_L$ values show excellent agreement with XFLR5 predictions across the operational angle-of-attack range, validating the computational model for lift estimation and confirming accurate force balance extraction.
  \item \textbf{Drag coefficients}: Measured $C_D$ values are consistently higher than the XFLR5 wing-only simulations, as expected since the simulation captures only the wing drag while the experimental data includes additional system-level drag sources such as fuselage parasite drag, propeller-induced drag, propeller-wing interference effects, and landing gear.
  \item \textbf{L/D ratios}: Experimental efficiency (L/D $\approx$ 0.5--1.8) is lower than isolated wing performance due to these additional drag contributions, but the trends validate the force balance approach and demonstrate measurable aerodynamic contribution from the wings.
\end{itemize}
...
To validate the platform's performance in realistic operational conditions beyond the controlled indoor environment, we conducted outdoor flight tests using manual control with GPS-based velocity measurement and onboard data logging via Betaflight blackbox. These tests provide critical insights into real-world flight characteristics including altitude tracking, motor loading dynamics, power consumption, and the practical efficiency gains achieved during cruise flight.

\subsection{Test Configuration and Methodology}

The outdoor tests were performed using the AG25-equipped platform (Wing~2, mass \SI{2.8}{\kg}) in open-field conditions with manual flight control and GPS-based velocity measurement. Flight data was recorded at \SI{1}{\kilo\hertz} using Betaflight's blackbox logging system, capturing:
...
\begin{itemize}
  ...
  \item Battery voltage and current consumption
\end{itemize}

A representative \SI{10}{\second} cruise segment (3--13\,s) at commanded \SI{10}{\meter\per\second} forward velocity was extracted for detailed analysis. This segment represents steady-state cruise conditions with stable altitude and velocity, providing clean data for efficiency characterization.

\subsection{Flight Performance Metrics}

Table~\ref{tab:outdoor_flight_summary} summarizes the key flight parameters extracted from the cruise segment.

\begin{table}[htbp]
\centering
\caption{Outdoor flight performance summary for \SI{10}{\meter\per\second} cruise segment (AG25 platform).}
\label{tab:outdoor_flight_summary}
\begin{tabular}{lc}
\hline
\textbf{Parameter} & \textbf{Value} \\
\hline
Cruise speed (GPS) & \SI{10.0}{\meter\per\second} \\
Cruise pitch angle & \SI{87.5}{\degree} \\
Hover RPM (upper quartile) & \SI{11957}{rpm} \\
Cruise RPM (lower quartile) & \SI{9539}{rpm} \\
RPM ratio (cruise/hover) & 0.798 \\
Estimated power savings & 49.2\% \\
Mean voltage & \SI{22.8}{\volt} \\
Mean current & \SI{8.5}{\ampere} \\
Mean power & \SI{194}{\watt} \\
\hline
\end{tabular}
\end{table}

The data reveals significant rotor unloading during cruise: the motors operate at approximately 80\% of hover RPM, translating to an estimated 49\% reduction in rotor power using the momentum theory scaling $P \propto \Omega^3$. This matches the indoor test results for the AG25 platform (Section~\ref{sec:airfoil_comparison}), validating that the efficiency gains transfer to real-world outdoor operation.

\subsection{Time-Series Analysis}

Figures~\ref{fig:outdoor_speed}--\ref{fig:outdoor_rpm} present detailed time-series data from the cruise segment, illustrating the dynamic behavior of the platform during sustained forward flight.
...
\begin{figure}[htbp]
  ...
  \addplot[black, dashed, thick] coordinates {(3, 10) (13, 10)};
  \addlegendentry{GPS Speed}
  \addlegendentry{Target 10 m/s}
  \end{axis}
  \end{tikzpicture}
  \caption{GPS velocity during outdoor cruise flight. The platform maintains steady \SI{10}{\meter\per\second} forward speed with minimal oscillation under manual control, demonstrating the pilot's ability to achieve consistent cruise conditions for efficiency testing.}
  \label{fig:outdoor_speed}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \addplot[tumblue, thick] table[x=time_s, y=altitude_rel_m, col sep=comma] {data/outdoor/outdoor_flight_motor_data.csv};
  \addplot[black, dashed, thick] coordinates {(3, 0) (13, 0)};
  \end{axis}
  \end{tikzpicture}
  \caption{Barometric altitude (relative to takeoff) during cruise. Small altitude variations ($\pm$\SI{0.5}{\meter}) indicate stable altitude hold despite outdoor wind disturbances.}
  \label{fig:outdoor_altitude}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \addplot[tumblue, thick] table[x=time_s, y=pitch_deg, col sep=comma] {data/outdoor/outdoor_flight_motor_data.csv};
  \addplot[black, dashed, thick] coordinates {(3, 0) (13, 0)};
  \end{axis}
  \end{tikzpicture}
  \caption{Pitch angle time history showing the vehicle attitude during cruise. The mean pitch of \SI{87.5}{\degree} (measured from horizontal) corresponds to approximately \SI{2.5}{\degree} nose-down from vertical, indicating near-level flight with wings generating the majority of lift.}
  \label{fig:outdoor_pitch}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \addplot[tumorange, thick] table[x=time_s, y=power_W, col sep=comma] {data/outdoor/outdoor_flight_motor_data.csv};
  \end{axis}
  \end{tikzpicture}
  \caption{Electrical power consumption during cruise flight. The mean power of \SI{194}{\watt} includes all systems (motors, flight controller, GPS, telemetry). Power variations correlate with altitude and velocity control inputs.}
  \label{fig:outdoor_power}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \addplot[gray, opacity=0.4] table[x=time_s, y=motor1_rpm, col sep=comma] {data/outdoor/outdoor_flight_motor_data.csv};
  \addplot[gray, opacity=0.4] table[x=time_s, y=motor2_rpm, col sep=comma] {data/outdoor/outdoor_flight_motor_data.csv};
  \addplot[gray, opacity=0.4] table[x=time_s, y=motor3_rpm, col sep=comma] {data/outdoor/outdoor_flight_motor_data.csv};
  \addplot[gray, opacity=0.4] table[x=time_s, y=motor4_rpm, col sep=comma] {data/outdoor/outdoor_flight_motor_data.csv};
  \addplot[TUMAccentOrange, thick, mark=none] table[x=time_s, y=motor_avg_rpm, col sep=comma] {data/outdoor/outdoor_flight_motor_data.csv};
  \addlegendentry{Individual Motors}
  \addlegendentry{}
  \addlegendentry{}
  \addlegendentry{}
  \addlegendentry{Average RPM}
  \end{axis}
  \end{tikzpicture}
  \caption{Motor RPM traces showing individual motor speeds (gray) and average (red). The tight clustering of individual motor speeds indicates balanced thrust distribution and effective attitude control. Periodic variations reflect pitch and roll corrections during flight.}
  \label{fig:outdoor_rpm}
\end{figure}

\subsection{Discussion and Validation}

The outdoor flight data validates several key design objectives:

\begin{itemize}
  \item \textbf{Efficiency confirmation}: The 49\% power savings matches indoor test predictions, demonstrating that the AG25 wing provides consistent efficiency gains across different flight environments.
  
  \item \textbf{Manual flight performance}: The platform achieves stable \SI{10}{\meter\per\second} cruise under manual control despite outdoor wind conditions, indicating good handling characteristics and the ability to maintain steady-state cruise flight.
  
  \item \textbf{Altitude stability}: Barometric altitude variations remain within $\pm$\SI{0.5}{\meter} during cruise, acceptable for outdoor flight applications and confirming effective manual altitude control.
  
  \item \textbf{Near-level flight}: The \SI{87.5}{\degree} pitch angle (only \SI{2.5}{\degree} from vertical) indicates the wings generate approximately 90\% of the required lift at cruise speed, with rotors primarily providing thrust and control authority.
  
  \item \textbf{Motor balance}: Individual motor RPMs cluster tightly around the mean with synchronized variations, indicating proper control allocation and minimal cross-coupling despite aerodynamic asymmetries.
\end{itemize}

These results demonstrate that the hybrid X-wing platform achieves its design goals in realistic outdoor conditions: efficient cruise flight through passive aerodynamic lift, stable flight characteristics under manual control, and robust disturbance rejection via the INDI controller. The measured 49\% power reduction compared to conventional quadcopter operation validates the platform's potential for extended-range missions requiring both VTOL capability and efficient cruise performance.
